...
def train(
    ...
    name_of_algorithm_used_in_pipeline = list(model_pipeline.named_steps)[
        len(list(model_pipeline.named_steps)) - 1
    ]
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        signature = infer_signature(
            X_train.sample(3, random_state=111),
            model_pipeline.predict(X_train.sample(3, random_state=111)),
        )
...
def prepare_model_pipeline(df: pd.DataFrame, model, model_params: dict) -> Pipeline:
    ...
    numeric_transformer = Pipeline(
        steps=[("scaler", StandardScaler())]  # Scale numeric features
    )
...
def run_train() -> None:
    ...
    random_forest_pipeline = prepare_model_pipeline(
        X_train, RandomForestRegressor, random_forest_params
    )

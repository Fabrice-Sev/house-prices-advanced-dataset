...
def make_data_processing(df_test: pd.DataFrame) -> pd.DataFrame:
    ...
    new_df_test["LotFrontage"] = df_test.groupby("Neighborhood")[
        "LotFrontage"
    ...
    new_df_test["BsmtExposure"] = df_test["BsmtExposure"].fillna(
        df_test["BsmtExposure"].mode()[0]
    )
    new_df_test["BsmtFinType2"] = df_test["BsmtFinType2"].fillna(
        df_test["BsmtFinType2"].mode()[0]
    )

    # # This will be to handle na_values for Electrical
    new_df_test["Electrical"] = df_test["Electrical"].fillna(
        df_test["Electrical"].mode()[0]
    )
    ...
    new_df_test.loc[new_df_test["HasBsmt"] == 1, "TotalBsmtSF"] = np.log(
        new_df_test.loc[new_df_test["HasBsmt"] == 1, "TotalBsmtSF"]
    )
...
def make_predictions(
    df_test: pd.DataFrame, model_name: str = "xgb_model"
) -> pd.DataFrame:
